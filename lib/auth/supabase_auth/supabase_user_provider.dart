...
class ChessmySupabaseUser extends BaseAuthUser {
  ChessmySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> chessmySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChessmySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
